 ...
 * @file lcut.h
 *
 * @brief a Lightweight C Unit Test framework,
...
#define LCUT_MAX_STR_LEN 256
#ifndef XBOOL_TRUE
    /** Boolean TRUE for xcut. */
    #define XBOOL_TRUE 1
#endif

#ifndef XBOOL_FALSE
  /** Boolean FALSE for xcut. */
  #define XBOOL_FALSE 0
#endif
...
#define SUCCESS_TIP_FMT "\x1b[0;32m [  Passed  ]\x1b[0m Case '%s'\n"
#define FAILURE_TIP_FMT "\x1b[0;31m [  Failed  ]\x1b[0m Case '%s':\
\n  *\x1b[1;35m File    : %s : %d : Failed\x1b[0m\n  *\x1b[1;35m Function: %s \x1b[0m \n%s\n"

#define REDBAR \
"\n==================================================\n\
\t\t\x1b[0;31m RED BAR! \x1b[0m \n\
==================================================\n"

#define GREENBAR \
"\n==================================================\n\
\t\t\x1b[0;32m GREEN BAR! \x1b[0m \n\
==================================================\n"
...
struct lcut_tc_t {
    ...
    int                         memcount;                   /* count the memory malloc ,alloc or free*/
};
...
typedef struct lcut_test_t {
    ...
    long                        costtime;                   /* the time run this test */
} lcut_test_t;
...
int stri_equal(const char *str1, const char *str2);
int str_equaln(const char *str1, const char *str2, int n);

int xcut_int_assert(lcut_tc_t *tc, int cValue, int lineno, const char *fcname,
                    const char *fname, int expected, int actual);

int xcut_str_assert(lcut_tc_t *tc, int cValue, int lineno, const char *fcname,
		const char *fname, const char* expected, const char* actual);

int lcut_assert(lcut_tc_t *tc, const char *msg, int condition,
...
#define LCUT_INT_NE(tc, expected, actual) do { \
        ...
        "LCUT_INT_NE"__, __FILE__, expected, actual); \
    } while(0)

#define LCUT_INT_GT(tc, expected, actual) do { \
        xcut_int_assert(tc, (expected) > (actual), __LINE__,\
        "LCUT_INT_GT", __FILE__, expected, actual); \
    } while(0)

#define LCUT_INT_LT(tc, expected, actual) do { \
        xcut_int_assert(tc, (expected) < (actual), __LINE__,\
        "LCUT_INT_LT", __FILE__, expected, actual); \
    } while(0)

#define LCUT_INT_GE(tc, expected, actual) do { \
        xcut_int_assert(tc, (expected) >= (actual), __LINE__,\
        "LCUT_INT_GE", __FILE__, expected, actual); \
    } while(0)

#define LCUT_INT_LE(tc, expected, actual) do { \
        xcut_int_assert(tc, (expected) <= (actual), __LINE__,\
        "LCUT_INT_LE", __FILE__, expected, actual); \
    } while(0)

#define LCUT_STR_EQ(tc, expected, actual) do { \
        xcut_str_assert(tc, str_equal(expected, actual), __LINE__,\
        "LCUT_STR_EQ", __FILE__, expected, actual); \
    } while(0)

#define LCUT_STR_CASEEQ(tc, expected, actual) do { \
        xcut_str_assert(tc, stri_equal(expected, actual), __LINE__,\
        "LCUT_STR_CASEEQ", __FILE__, expected, actual); \
    } while(0)

#define LCUT_STR_CASENE(tc, expected, actual) do { \
        xcut_str_assert(tc, !stri_equal(expected, actual), __LINE__,\
        "LCUT_STR_CASENE", __FILE__, expected, actual); \
    } while(0)

#define LCUT_STR_NE(tc, expected, actual) do { \
        xcut_str_assert(tc, !str_equal(expected, actual), __LINE__,\
        "LCUT_STR_NE", __FILE__, expected, actual); \
    } while(0)

#define LCUT_STR_EN(tc, expected, actual, n) do { \
        xcut_str_assert(tc, str_equaln(expected, actual, n), __LINE__,\
        "LCUT_STR_EN", __FILE__, (expected " : Length: " #n), actual); \
    } while(0)

#define LCUT_PTR_EQ(tc, expected, actual) do { \
        xcut_str_assert(tc, ((void *)expected == (void *) actual), __LINE__,\
        "LCUT_PTR_EQ", __FILE__, expected, actual); \
    } while(0)

#define LCUT_DOU_EQ(actual, expected, granularity) do {\
        xcut_str_assert(tc, ((fabs((double)(actual) - (expected)) <= fabs((double)(granularity)))),\
        __LINE__, "LCUT_DOU_EQ", __FILE__,  (#expected " : #granularity: " #granularity ), #actual);\
    } while(0)

#define LCUT_DOU_NE(actual, expected, granularity) do {\
        xcut_str_assert(tc, ((fabs((double)(actual) - (expected)) > fabs((double)(granularity)))),\
        __LINE__, "LCUT_DOU_NE", __FILE__,  (#expected " : #granularity: " #granularity ), #actual);\
    } while(0)

#define LCUT_PTR_NE(tc, expected, actual) do { \
        xcut_str_assert(tc, ((void *)expected != (void *) actual), __LINE__, "LCUT_PTR_NE",\
        __FILE__, #expected, actual); \
    } while(0)

#define LCUT_ASSERT(tc, msg, condition) do { \
        lcut_assert(tc, msg, (condition), __LINE__, "LCUT_ASSERT", __FILE__); \
    } while(0)

#define LCUT_TRUE(tc, condition) do { \
        lcut_assert(tc, NULL, condition, __LINE__, "LCUT_TRUE", __FILE__); \
    } while(0)

#define LCUT_FALSE(tc, condition) do { \
        lcut_assert(tc, NULL, !condition, __LINE__, "LCUT_FALSE", __FILE__); \
    } while(0)
